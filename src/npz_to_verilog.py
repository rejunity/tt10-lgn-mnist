...
def generate_verilog_testdata(testdata_input, testdata_output, modulename="net"):
    X = testdata_input
    Y = testdata_output
    ...
    MAX_INPUT_SAMPLES = 64 # limit to 64 input samples for speed
    n = min(MAX_INPUT_SAMPLES, X.shape[0])
    in_w = X.shape[1]
    out_w = Y.shape[1]
...
`timescale 1ns/1ps
`define TESTDATA_N {n}
`define TESTDATA_X {in_w}
`define TESTDATA_Y {out_w}
module {modulename}_testdata;
  parameter N = `TESTDATA_N;
  reg  [(`TESTDATA_X)-1:0] X [0:(`TESTDATA_N)-1];
  reg  [(`TESTDATA_Y)-1:0] Y [0:(`TESTDATA_N)-1];  
  initial begin
{init_block}
  end
endmodule
...
if __name__ == "__main__":
    ...
    if GENERATE_TESTBENCH_DATA and "input" in data and "output" in data:
        testbench_file_name = verilog_file_name[:-2] + "_testdata.v"
        testbench = f"// Generated from: {npz_file_name}\n" + \
            generate_verilog_testdata(data["input"], data["output"])
