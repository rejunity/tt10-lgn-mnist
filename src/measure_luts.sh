...
PTH=../__paper2_pth/
LUTN=4
XILINX_FPGA=~/nextpnr-xilinx/xilinx/xc7a200t.bin
XILINX_XDC=../fpga/xilinx/xilinx-xc7a200t.xdc
...
# time yosys -p "read_verilog ${CFG}.v ../fpga/xilinx/xilinx.v project.v net.v; synth_xilinx -abc9 -top top; write_json xilinx_${NAME}.json" 2>&1 | tee ${TYPE}_LUTX6_${NAME}.log | grep -E "ERROR|^===|^--|LUT|LCs:"
# time nextpnr-xilinx --json xilinx_${NAME}.json --chipdb $XILINX_FPGA --xdc $XILINX_XDC 2>&1 | tee XILINX_${TYPE}_${NAME}.log | grep -E "ERROR|^===|^--|LCs:|frequency|routing"
# time yosys -p "read_verilog ${CFG}_countbits.v project.v net.v; synth_xilinx -abc9 -top tt_um_rejunity_lgn_mnist -json xilinx_${NAME}.json" 2>&1 | tee ${TYPE}_LUTX6_CB_${NAME}.log | grep -E "ERROR|^===|^--|LUT|LCs:"
...
# time yosys -p "read_verilog ${CFG}.v project.v net.v; synth_ice40 -top tt_um_rejunity_lgn_mnist -json ice40.json" 2>&1 | tee ${TYPE}_LUTC4_${NAME}.log | grep -E "ERROR|^===|^--|LUT|LCs:"
time yosys -p "read_verilog ${CFG}.v ../fpga/xilinx/xilinx.v project.v net.v; synth_xilinx -abc9 -top top; write_json xilinx_${NAME}.json" 2>&1 | tee ${TYPE}_LUTX6_${NAME}.log | grep -E "ERROR|^===|^--|LUT|LCs:"
time nextpnr-xilinx --json xilinx_${NAME}.json --chipdb $XILINX_FPGA --xdc $XILINX_XDC | tee XILINX_${TYPE}_${NAME}.log | grep -E "ERROR|^===|^--|LCs:|frequency|routing"
# time yosys -p "tcl ../fpga/generic/synth_lut$LUTN/synth_generic.tcl $LUTN ../fpga/generic/generic.json" ${CFG}.v project.v net.v 2>&1 | tee ${TYPE}_LUT${LUTN}_${NAME}.log | grep -E "ERROR|^===|^--|LUT"
